...
**How you find "the next phase":** walk the file's `### Phase N:` sections in order and take the first one with at least one task in `- [ ]` (ignore optional `[ ]*` ones if everything else in that phase is already `[x]`). If none remain, say so — the file is complete.
...
3. Check the repo's state: `git status`. If you're on the main branch, **tell me and wait** — no direct work on main.
   If there are uncommitted changes, show me `git status --short` and **ask me once** whether it's work from an earlier phase of this same task file. Don't try to guess the origin on your own — asking is cheaper and more reliable. If I confirm yes, continue; if not, stop.
...
If a task fails twice on the model from its assigned `_Model:_`, retry one tier up (`haiku`→`sonnet`, `sonnet`→`opus`; `opus` doesn't escalate further, it's already the ceiling — full table in `${CLAUDE_PLUGIN_ROOT}/harness/CONVENCIONES.md`) **before** invoking the Step 2 advisor. If the escalated task succeeds, the escalation is recorded in that task's **single** row in the run log — with the final model and the escalation visible there, not in an extra row (format in `${CLAUDE_PLUGIN_ROOT}/harness/CONVENCIONES.md`).
...
- The same test or error keeps failing **after 2 fix attempts**.
...
- If a task turns out to be impossible or was already done, don't mark it: leave it in `[ ]`, note it in chat, and move on.
